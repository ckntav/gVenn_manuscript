...
# Note:    Overlaps are computed with gVenn::computeOverlaps() in "reduce" mode
...
# mode = "reduce" (the default) merges overlapping peaks within each set before
# intersecting them, so every region belongs to a single overlap group
ov <- computeOverlaps(demo_peaks, mode = "reduce")
...
demo_reduced_regions <- ov$regions
...
output_dir <- here("output", "overlaps_reduced_bed")
dir.create(output_dir, recursive = TRUE, showWarnings = FALSE)

rtracklayer::export(demo_reduced_regions,
                    format = "bed",
                    con = file.path(output_dir, "example_reduced_regions.bed"))
...
exportOverlapsToBed(overlapGroups,
                    output_dir = output_dir,    # Same directory as the reduced regions
                    output_prefix = "example_reduced",  # Prefix for output filenames
                    with_date = FALSE)          # Don't append date to filenames

# ==============================================================================
# Output files generated in output/overlaps_reduced_bed/:
