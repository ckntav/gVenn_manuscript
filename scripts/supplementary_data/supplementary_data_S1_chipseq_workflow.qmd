...
By default, `computeOverlaps()` uses `mode = "reduce"`: overlapping peaks from all sets are merged into reduced regions, and each region is labeled with the sets that contributed an interval somewhere within it. With `mode = "disjoin"`, the union of peaks is instead cut at every set boundary into disjoint regions, whose label holds at every position they cover.
In either mode, the numbers reported below are counts of regions, not of input peaks, so a set's total can be lower than its number of peaks when several of its peaks fall within the same region.
...
`stress` and `diagError` are `0` for a perfect fit, and larger values indicate greater geometric distortion. `diagError` is the largest single-region discrepancy, over all regions, between the share of the total diagram area a region is given and the share the counts require, a proportion between 0 and 1. `regionError` reports that discrepancy region by region. `undrawnRegions` names the regions the counts populate but the layout gives no area to at all.

Micallef and Rodgers (2014), who introduced diagError, call a diagram accurately proportional to its data when it is at most 1e-6, and that is the threshold the message reports against. Here both statistics are very close to zero without being exactly zero and no region is undrawn, so the drawn areas match the underlying counts closely.

Fit quality degrades as sets are added, because the solver has fewer degrees of freedom to satisfy every intersection at once. These values are therefore worth checking before interpreting the areas of a four-set or larger diagram.
Set `verbose = FALSE` to silence the message; the attribute is attached either way.
See the [eulerr documentation](https://jolars.github.io/eulerr/) for the exact definitions of these statistics.
...
The top bar chart shows intersection sizes for each overlap combination, while the connected dots below indicate which sets participate in each intersection. Horizontal bars on the right show the total number of regions per set. The UpSet representation is particularly useful for comparing four or more sets or when precise quantification of complex overlap patterns is required.
