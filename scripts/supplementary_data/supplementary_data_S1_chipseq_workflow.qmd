...
`stress` and `diagError` are `0` for a perfect fit, and larger values indicate greater geometric distortion. `diagError` is the largest single-region discrepancy, in percentage points of total area, and `regionError` reports that discrepancy for each region separately. Here both are very close to zero without being exactly zero, so the drawn areas match the underlying counts closely.
