...
The Bioconductor release version may lag behind development, so the newest features are not always available there.
...
This workflow requires gVenn version 1.99.0 or later. Earlier versions do not provide all the functions used below.
...
By default, `computeOverlaps()` uses `mode = "reduce"`: overlapping peaks from all sets are merged into reduced regions, and each region is labeled with the sets that contributed an interval somewhere within it.

With `mode = "disjoin"`, the union of peaks is instead cut at every set boundary into disjoint regions, whose label holds at every position they cover.

In either mode, the numbers reported below are counts of regions, not of input peaks.

```{r}
#| label: compute-overlaps-disjoin

# Same peak sets, partitioned at every set boundary
genomic_overlaps_disjoin <- computeOverlaps(a549_chipseq_peaks, mode = "disjoin")

# Number of regions under each mode
c(reduce  = length(genomic_overlaps$regions),
  disjoin = length(genomic_overlaps_disjoin$regions))
```
...
The message above reports `eulerr`'s goodness-of-fit diagnostics for this layout.

```{r}
#| label: fig-venn-basic2
...
`plotVenn()` also attaches them to the object it returns, together with the per-region errors:

```{r}
#| label: venn-fit-diagnostics

attr(venn, "fit_diagnostics")
```

`diagError` is the largest difference, over all regions, between the share of the diagram's area a region receives and the share its count requires: 0 for an exact fit, at most 1. `regionError` gives that difference region by region, and `undrawnRegions` names any non-empty region left with no area. `stress` is `eulerr`'s own loss function, also 0 for an exact fit.

Following Micallef and Rodgers (2014), who introduced `diagError`, a diagram is considered accurate when it draws every non-empty region and its `diagError` does not exceed 10⁻⁶, the threshold the message reports against. Here `diagError` is `r sprintf("%.3e", attr(venn, "fit_diagnostics")$diagError)` and no region is undrawn.

With four sets, ellipses cannot produce every combination of the fifteen region areas, and an exact fit rarely exists. Approximate areas matter little, since the count printed in each region carries the quantitative information. An undrawn region is more serious, since its count disappears from the figure. In that case, or when the fit is poor, use `plotUpSet()` (next section).

Set `verbose = FALSE` to silence the message; the attribute is attached either way. See the [eulerr documentation](https://jolars.github.io/eulerr/) for the definition of `stress`.

## UpSet plot

`plotUpSet()` shows the same overlap groups as bars along a common scale, so every count stays readable whatever the number of sets. It is the alternative when a Venn diagram leaves a region undrawn or fits poorly, and the only option beyond four sets.

```{r}
#| label: fig-upset
#| fig-cap: "UpSet plot of the overlap groups between MED1, BRD4 and GR regions (mode = \"reduce\")"
#| fig-width: 6
#| fig-height: 3

# Colors follow the overlap groups of Figure 1: 111, 110, 101, 011, 100, 010, 001
plotUpSet(genomic_overlaps,
          comb_col = c("#D87093", "#CD3301", "#9370DB", "#008B8B",
                       "#2B70AB", "#FFB027", "#3EA742"))
```

The top bars give the number of regions in each overlap group, and the dots below indicate which sets form it. The horizontal bars, labeled "Region size", give the number of reduced regions per set, not the number of input peaks.
...
By default, files are written to the current directory (`"."`) and the current date is prepended to the filename; set `with_date = FALSE` to turn this off.
...
plotVenn(genomic_overlaps,
         ...
         quantities = list(type = c("counts","percent"),
                           col = "black", fontsize = 6),
         main = list(label = "Counts + Percentages", fontsize = 11))
```
